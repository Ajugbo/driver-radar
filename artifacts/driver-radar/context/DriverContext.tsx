...
import { loginDriver, registerDriver } from '@workspace/api-client-react';
...
const defaultFilters: Filters = {
  ...
  maxRadius: 3,
  minRating: null,
  blacklistedZones: ['Airport Road'],
};

const defaultPlatforms: Record<PlatformName, boolean> = {
  Uber: true,
  Bolt: true,
  inDrive: true,
};

const demoDriver: DriverIdentity = {
  id: 0,
  email: 'demo@driverradar.ng',
...
function makeMockRide(index: number): Ride {
  ...
  const ride = options[index % options.length];
  return {
    ...ride,
...
function formatStoredState(value: Partial<{
  rides: Ride[];
  filters: Filters;
  platforms: Record<PlatformName, boolean>;
  token: string | null;
  ...
  return {
    rides: value.rides ?? seedRides,
    filters: value.filters ?? defaultFilters,
    platforms: value.platforms ?? defaultPlatforms,
    token: value.token ?? null,
    driver: value.driver ?? demoDriver,
  };
}
...
export function DriverProvider({ children }: { children: ReactNode }) {
  ...
  const [platforms, setPlatforms] = useState<Record<PlatformName, boolean>>(defaultPlatforms);
  const [driver, setDriver] = useState<DriverIdentity>(demoDriver);
    ...
    AsyncStorage.getItem(STORAGE_KEY)
      .then((stored) => {
        ...
        if (stored) {
          ...
          const state = formatStoredState(parsed);
  ...
  useEffect(() => {
    ...
    AsyncStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({ rides, filters, platforms, token, driver }),
    ).catch(() => undefined);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setRides((current) => [makeMockRide(index++), ...current].slice(0, 12));
    ...
    () =>
      rides.filter(
        (ride) =>
          ...
          ride.fare >= filters.minFare &&
          ride.distance <= filters.maxRadius &&
  ...
  const toggleListener = () => {
    setIsListenerLive((current) => !current);
  ...
  const authenticate = async (mode: 'login' | 'register', email: string, password: string) => {
    const response =
      mode === 'login'
        ...
        : await registerDriver({ email, password });
    setToken(response.token);
    setDriver({
      id: response.driver.id,
      email: response.driver.email,
      subscriptionTier: response.driver.subscriptionTier,
    });
  };
  ...
  const value = useMemo<DriverContextValue>(
    () => ({
      ...
      isHydrated,
      isListenerLive,
      isPro: driver.subscriptionTier === 'pro',
      lastSync,
      updateFilters,
      togglePlatform,
      toggleListener,
      acceptRide: (id) => markRide(id, 'accepted'),
      declineRide: (id) => markRide(id, 'declined'),
